...
static void APP_UartHalApiTest(void)
{
	UART_InitStruct *uartStruct;

	HAL_UART_DeInit(UART_1);

	HAL_UART_StructInit(uartStruct);
	...
	HAL_UART_Init(UART_1, uartStruct);
...
static void APP_UartTest(void)
{
	...
	uartStruct->baud = UART_BAUD_115200;
	uartStruct->fracx = 2;		//24MHz, 115200bps ---> Refer UART register map formula!
	uartStruct->fracy = 10;
	uartStruct->triggerLevel = UART_TRIGGER_LEVEL_1;
	uartStruct->wordLength = UART_WORD_LENGTH_8BIT;
	uartStruct->stopBit = UART_STOP_BIT_1BIT;
	...
	uartStruct->stickParityMode = UART_STICK_PARITY_LOGIC_0;
	HAL_UART_Init(UART_1, uartStruct);
	HAL_UART_Init(UART_3, uartStruct);
